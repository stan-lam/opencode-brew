...
mod implementation {
    ...
    fn chunk_content(content: &str, file_path: &str) -> Vec<CodeChunk> {
        ...
        while start < lines.len() {
            let end = (start + CHUNK_SIZE / 30).min(lines.len());
    ...
    async fn get_embeddings(texts: &[String], ollama_url: &str) -> Result<Vec<Vec<f32>>, String> {
        ...
        let response = client
            ...
            .send()
            .await
            .map_err(|e| format!("Failed to connect to Ollama: {}. Make sure Ollama is running and {} model is installed", e, EMBEDDING_MODEL))?;
    ...
    pub async fn index_workspace_impl(workspace_path: String, ollama_url: Option<String>) -> Result<super::IndexStatus, String> {
    ...
    pub async fn search_codebase_impl(
    ...
    pub async fn get_index_status_impl(workspace_path: String) -> Result<super::IndexStatus, String> {
        let db_path = get_db_path(&workspace_path);
    ...
    pub async fn delete_index_impl(workspace_path: String) -> Result<(), String> {
        let db_path = get_db_path(&workspace_path);
...
pub async fn index_workspace(workspace_path: String, ollama_url: Option<String>) -> Result<IndexStatus, String> {
    #[cfg(feature = "vectordb")]
    {
        implementation::index_workspace_impl(workspace_path, ollama_url).await
    }
    #[cfg(not(feature = "vectordb"))]
    {
        let _ = (workspace_path, ollama_url);
        Err("Vector database feature is not enabled. Enable 'vectordb' feature to use semantic search.".to_string())
    }
}
...
pub async fn search_codebase(
    ...
    #[cfg(feature = "vectordb")]
    {
        implementation::search_codebase_impl(workspace_path, query, limit, ollama_url).await
    }
    #[cfg(not(feature = "vectordb"))]
    {
        let _ = (workspace_path, query, limit, ollama_url);
        Err("Vector database feature is not enabled. Enable 'vectordb' feature to use semantic search.".to_string())
    }
}
...
pub async fn get_index_status(workspace_path: String) -> Result<IndexStatus, String> {
    #[cfg(feature = "vectordb")]
    {
        implementation::get_index_status_impl(workspace_path).await
    }
    #[cfg(not(feature = "vectordb"))]
    {
        let hash = md5::compute(&workspace_path);
        let home = dirs::home_dir().unwrap_or_else(|| std::path::PathBuf::from("."));
        let db_path = home.join(".opencodebrew").join("vector_indexes").join(format!("{:x}", hash)).to_string_lossy().to_string();
        Ok(IndexStatus {
            is_indexed: false,
            file_count: 0,
            last_indexed: None,
...
pub async fn delete_index(workspace_path: String) -> Result<(), String> {
    #[cfg(feature = "vectordb")]
    {
        implementation::delete_index_impl(workspace_path).await
    }
    #[cfg(not(feature = "vectordb"))]
    {
